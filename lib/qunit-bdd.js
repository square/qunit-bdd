...
void function(global, exports, QUnit, options) {
  ...
  /**
   * QUnit stores all hooks in outside-first, top-to-bottom order.
   * QUnit reverses the order of hooks for "after" and "afterEach" queues,
   * giving those queues inside-first, bottom-to-top behavior.
   *
   * @private
   * @param {string} hook The name of the hook, such as "before".
   */
  Context.prototype.gatherHooks = function(hook) {
    var queue = [];
  ...
  function addTestsToQUnit() {
    if (QUnit.config.currentModule) {
      throw new Error('qunit-bdd cannot start due to a failure processing the module: ' + QUnit.config.currentModule.name);
    }
    ...
    pendingContexts.forEach(function(context) {
      ...
      var beforeHooks = context.gatherHooks('before');
      var afterHooks = context.gatherHooks('after');
      var lazySetupHooks = context.gatherHooks('lazySetup');
      var lazyTeardownHooks = context.gatherHooks('lazyTeardown');
      ...
      // ensure lazy setup hooks are the very first thing to run and
      // lazy teardown hooks are the very last thing to run
      currentHooks.beforeEach = (lazySetupHooks || []).concat(currentHooks.beforeEach || []);
      currentHooks.afterEach = (lazyTeardownHooks || []).concat(currentHooks.afterEach || []);
